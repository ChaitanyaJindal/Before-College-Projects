...
class Bill_App:
    def __init__(self,root):
        ...
        c1_lbl=Label(F4,text="Mazaa",font=("times new roman",15,"bold"),bg=bg_color,fg="lightgreen").grid(row=0,column=0,padx=10,pady=10,sticky="w")
        c1_txt=Entry(F4,width=10,textvariable=self.mazaa,font=("times new roman",12,"bold"),bd=5,relief=SUNKEN).grid(row=0,column=1,padx=8,pady=10)
    ...
    def welcome_bill(self):
        ...
        self.textarea.insert(END,"\t Welcome Easy Retail")
    ...
    def bill_area(self):
        if self.c_name.get()=="" or self.c_phon.get()=="":
            ...
            #===Cold Drink=====
            if self.mazaa.get()!=0:
                self.textarea.insert(END,f"\n Mazaa   \t\t{self.mazaa.get()}\t\t{self.cd_m_p}")
            if self.coke.get()!=0:
